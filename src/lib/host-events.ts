...
const HOST_EVENT_TO_IPC_CHANNEL: Record<string, string> = {
  ...
  'channel:openzalo-qr': 'channel:openzalo-qr',
  'channel:openzalo-success': 'channel:openzalo-success',
  'channel:openzalo-error': 'channel:openzalo-error',
};
...
function allowSseFallback(): boolean {
  try {
    return window.localStorage.getItem('amybot:allow-sse-fallback') === '1';
  } catch {
    return false;
  }
}

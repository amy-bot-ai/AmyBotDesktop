...
export function AgentFilesTab() {
  ...
  const fetchMeta = useCallback(async () => {
    setLoadingMeta(true);
    try {
      const data = await hostApiFetch<WorkspaceFilesResponse>('/api/workspace-files');
  ...
  const fetchContent = useCallback(async (key: string, filename: string) => {
    ...
    try {
      const data = await hostApiFetch<FileContentResponse>(`/api/workspace-files/${encodeURIComponent(filename)}`);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await hostApiFetch(`/api/workspace-files/${encodeURIComponent(activeFile.filename)}`, {
        method: 'PUT',

...
<a href="#почему-amybot">Почему AmyBot</a> •
...
**AmyBot** — это мост между мощными AI-агентами и повседневными пользователями. Построенный на базе [OpenClaw](https://github.com/OpenClaw), он превращает управление AI через командную строку в доступный и красивый десктоп-опыт — терминал не нужен.

Автоматизация рабочих процессов, управление AI-каналами или планирование интеллектуальных задач — AmyBot предоставляет интерфейс для эффективного использования AI-агентов.

AmyBot поставляется с предустановленными лучшими практиками для провайдеров моделей и нативно поддерживает Windows, а также многоязычные настройки. Вы можете тонко настроить расширенные параметры через **Настройки → Дополнительно → Режим разработчика**.
...
Создание AI-агентов не должно требовать владения командной строкой. Философия AmyBot проста: **мощные технологии заслуживают интерфейса, который уважает ваше время.**

| Проблема | Решение AmyBot |
|----------|---------------|
...
AmyBot построен непосредственно на официальном ядре **OpenClaw**. Вместо отдельной установки мы встраиваем среду выполнения в приложение для бесшовного опыта "всё включено".
...
При выборе другого агента через `@agent` AmyBot переключается непосредственно в контекст этого агента вместо ретрансляции через агента по умолчанию. Рабочие пространства агентов по умолчанию разделены, но более строгая изоляция зависит от настроек песочницы OpenClaw.
...
Для пользовательских идентификаторов учётных записей каналов AmyBot требует совместимый с OpenClaw канонический формат (`[a-z0-9_-]`, строчные буквы, максимум 64 символа, должен начинаться с буквы или цифры) для предотвращения ошибок маршрутизации.
AmyBot также включает официальный плагин личного WeChat от Tencent, позволяя подключить WeChat напрямую со страницы Каналы через встроенный QR-код.
...
Когда совместимый шлюз отклоняет `/models` по причинам, не связанным с аутентификацией, AmyBot автоматически переключается на легковесный зонд `/chat/completions` или `/responses` при проверке API-ключа.

### 🌙 Адаптивные темы
Светлая тема, тёмная тема или синхронизация с системой. AmyBot автоматически адаптируется к вашим предпочтениям.

### 🚀 Управление автозапуском
В **Настройки → Общие** вы можете включить **Запускать при старте системы**, чтобы AmyBot автоматически запускался после входа в систему.
...
Скачайте последний релиз для вашей платформы со страницы [Releases](https://github.com/ValueCell-ai/AmyBot/releases).
...
При первом запуске AmyBot **Мастер настройки** проведёт вас через:
...
AmyBot включает встроенные настройки прокси для сред, где Electron, шлюз OpenClaw или каналы вроде Telegram должны выходить в интернет через локальный прокси-клиент.
...
- Если расширенные поля прокси пусты, AmyBot использует `Прокси-сервер`.
- Сохранение настроек прокси немедленно повторно применяет сеть Electron и автоматически перезапускает шлюз.
- AmyBot также синхронизирует прокси с конфигурацией канала Telegram в OpenClaw, когда Telegram включён.
- При перезапуске шлюза существующий прокси канала Telegram сохраняется, если прокси AmyBot отключен.
...
AmyBot использует **двухпроцессную архитектуру с унифицированным уровнем Host API**. Рендерер обращается к единой абстракции клиента, а Electron Main управляет выбором протокола и жизненным циклом процессов:

```
┌─────────────────────────────────────────────────────────────────┐
│                     Десктоп-приложение AmyBot                    │
│                                                                 │
│  ┌────────────────────────────────────────────────────────────┐ │
...
- AmyBot — это приложение Electron, поэтому **один экземпляр приложения обычно отображается как несколько процессов ОС** (main/renderer/zygote/utility). Это нормально.
...
- Нажатие кнопки закрытия окна (`X`) скрывает AmyBot в трей; это **не** полностью закрывает приложение. Используйте меню трея **Quit AmyBot** для полного завершения.
...
- использует временный каталог `userData` AmyBot
...
Мы приветствуем вклад сообщества! Исправления багов, новые функции, улучшения документации или переводы — каждый вклад делает AmyBot лучше.
...
AmyBot построен на плечах отличных проектов с открытым исходным кодом:
...
### Партнёрская программа AmyBot 🚀

Мы запускаем Партнёрскую программу AmyBot и ищем партнёров, которые могут помочь представить AmyBot большему числу клиентов, особенно тем, у кого есть потребности в кастомных AI-агентах или автоматизации.

Партнёры помогают связывать нас с потенциальными пользователями и проектами, а команда AmyBot предоставляет полную техническую поддержку, кастомизацию и интеграцию.
...
AmyBot выпускается под [лицензией MIT](LICENSE). Вы можете свободно использовать, модифицировать и распространять это программное обеспечение.

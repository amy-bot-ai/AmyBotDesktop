...
(function () {
  ...
  if (globalThis.__amybotFetchPatched) return;
  globalThis.__amybotFetchPatched = true;

  globalThis.fetch = function amybotFetch(input, init) {
    var url =
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__amybotPatched) {
        cp.__amybotPatched = true;

...
interface SyncProxyOptions {
   ...
   * currently disabled in AmyBot settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync AmyBot global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because AmyBot proxy is disabled and preserve mode is enabled');
      return;
    }

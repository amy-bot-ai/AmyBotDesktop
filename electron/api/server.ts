...
export function startHostApiServer(ctx: HostApiContext, port = getPort('AMYBOT_HOST_API')): Server {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EACCES' || error.code === 'EADDRINUSE') {
      logger.error(
        ...
        `Set AMYBOT_PORT_AMYBOT_HOST_API env var to override the default port.`,
      );
    } else {

...
import {
  ...
  OPENCLAW_ZALO_CHANNEL_TYPE,
  UI_WECHAT_CHANNEL_TYPE,
  UI_ZALO_CHANNEL_TYPE,
...
// ── OpenZalo QR login ────────────────────────────────────────────────────────
// QR is generated by the openclaw zalouser SDK (direct, no gateway needed).
// Config is saved as channels.zalouser (OPENCLAW_ZALO_CHANNEL_TYPE) which is
// the internal name the gateway uses — UI shows it as 'openzalo'.

const OPENZALO_QR_TIMEOUT_MS = 60 * 1000;

async function startOpenZaloQrLogin(
  accountId: string,
): Promise<{ qrDataUrl?: string; message: string }> {
  return startZaloQrLogin({ profile: accountId, force: true });
}

async function awaitOpenZaloQrLogin(
  ctx: HostApiContext,
  accountId: string,
  loginKey: string,
): Promise<void> {
  try {
    const result = await waitForZaloQrLogin({
      profile: accountId,
      timeoutMs: OPENZALO_QR_TIMEOUT_MS,
    });

    if (!isActiveQrLogin(loginKey, accountId)) return;

    if (!result.connected) {
      emitChannelEvent(ctx, UI_ZALO_CHANNEL_TYPE, 'error', result.message || 'Zalo login did not complete');
      return;
    }

    // Save under channels.openzalo (user-facing key, same as web portal)
    // Gateway will create channels.zalouser internally after reload
    await saveChannelConfig(UI_ZALO_CHANNEL_TYPE, { enabled: true }, accountId);
    await ensureScopedChannelBinding(UI_ZALO_CHANNEL_TYPE, accountId);
    // Restart covers both openzalo (config key) and zalouser (gateway internal)
    scheduleGatewayChannelSaveRefresh(ctx, UI_ZALO_CHANNEL_TYPE, `openzalo:loginSuccess:${accountId}`);

    if (!isActiveQrLogin(loginKey, accountId)) return;

    emitChannelEvent(ctx, UI_ZALO_CHANNEL_TYPE, 'success', {
      accountId,
      message: result.message || 'Zalo account connected',
    });
  } catch (error) {
    if (!isActiveQrLogin(loginKey, accountId)) return;
    emitChannelEvent(ctx, UI_ZALO_CHANNEL_TYPE, 'error', String(error));
  } finally {
    clearActiveQrLogin(UI_ZALO_CHANNEL_TYPE, accountId);
  }
}
...
async function buildChannelAccountsView(ctx: HostApiContext): Promise<ChannelAccountsView[]> {
  ...
  // Merge views that share the same uiChannelType.
  // This happens when the config key and the gateway's internal key both map to
  // the same UI type, e.g. 'openzalo' (config) and 'zalouser' (gateway runtime)
  // both become 'openzalo' via toUiChannelType.
  // The config view has the user-defined accounts; the runtime view has connected status.
  // We need to combine them so the UI shows the correct status.
  const mergedMap = new Map<string, ChannelAccountsView>();
  for (const view of channels) {
    const existing = mergedMap.get(view.channelType);
    if (!existing) {
      mergedMap.set(view.channelType, view);
      continue;
    }
    // Merge accounts — prefer existing entries, add runtime-only ones
    const existingIds = new Set(existing.accounts.map((a) => a.accountId));
    const mergedAccounts = [
      ...existing.accounts,
      ...view.accounts.filter((a) => !existingIds.has(a.accountId)),
    ];
    // Re-compute status across all merged accounts
    const allRuntime = mergedAccounts.map((a) => ({
      connected: a.connected,
      running: a.running,
      linked: a.linked,
      lastError: a.lastError,
    }));
    mergedMap.set(view.channelType, {
      ...existing,
      accounts: mergedAccounts,
      status: pickChannelRuntimeStatus(allRuntime),
      // Prefer the defaultAccountId from whichever view has a real one
      defaultAccountId: existing.defaultAccountId !== 'default' ? existing.defaultAccountId : view.defaultAccountId,
    });
  }

  return Array.from(mergedMap.values()).sort((left, right) => left.channelType.localeCompare(right.channelType));
}
...
export async function handleChannelRoutes(
  ...
  if (url.pathname === '/api/channels/openzalo/start' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ accountId?: string }>(req);
      const accountId = body.accountId?.trim() || 'default';

      const startResult = await startOpenZaloQrLogin(accountId);
      if (!startResult.qrDataUrl) {
        sendJson(res, 500, { success: false, error: startResult.message || 'Failed to generate Zalo QR code' });
        return true;
      }

      const loginKey = setActiveQrLogin(UI_ZALO_CHANNEL_TYPE, accountId, accountId);
      emitChannelEvent(ctx, UI_ZALO_CHANNEL_TYPE, 'qr', {
        qr: startResult.qrDataUrl,
        raw: startResult.qrDataUrl,
      });
      void awaitOpenZaloQrLogin(ctx, accountId, loginKey);
      sendJson(res, 200, { success: true });
    } catch (error) {
      sendJson(res, 500, { success: false, error: String(error) });
    }
  ...
  if (url.pathname === '/api/channels/openzalo/cancel' && req.method === 'POST') {
    try {
      const body = await parseJsonBody<{ accountId?: string }>(req);
      const accountId = body.accountId?.trim() || 'default';
      clearActiveQrLogin(UI_ZALO_CHANNEL_TYPE, accountId);

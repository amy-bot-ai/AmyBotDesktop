...
import { ensureAmyBotContext, repairAmyBotOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.amybot.desktop';
const isE2EMode = process.env.AMYBOT_E2E === '1';
const requestedUserDataDir = process.env.AMYBOT_USER_DATA_DIR?.trim();
...
// On Wayland this maps the running window to amybot.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('amybot.desktop');
}
...
if (!gotElectronLock) {
  console.info('[AmyBot] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[AmyBot] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[AmyBot] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
function createWindow(): BrowserWindow {
  ...
  const shouldSkipSetupForE2E = process.env.AMYBOT_E2E_SKIP_SETUP === '1';
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== AmyBot Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain AmyBot markers (no OpenClaw
  // template content). This fixes a race condition where ensureAmyBotContext()
  // previously created the file before the gateway could seed the full template.
  if (!isE2EMode) {
    void repairAmyBotOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running' && !isE2EMode) {
      void ensureAmyBotContext().catch((error) => {
        logger.warn('Failed to re-merge AmyBot context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge AmyBot context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureAmyBotContext will retry until the target files appear.
  if (!isE2EMode) {
    void ensureAmyBotContext().catch((error) => {
      logger.warn('Failed to merge AmyBot context into workspace:', error);
    });
  }
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second AmyBot instance detected; redirecting to the existing window');

...
# Post-installation script for AmyBot on Linux
...
# Create symbolic link for AmyBot app binary
if [ -x /opt/AmyBot/amybot ]; then
    ln -sf /opt/AmyBot/amybot /usr/local/bin/amybot 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/AmyBot/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/AmyBot/chrome-sandbox' || true
else
    chmod 0755 '/opt/AmyBot/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/AmyBot/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/amybot'
...
echo "AmyBot has been installed successfully."

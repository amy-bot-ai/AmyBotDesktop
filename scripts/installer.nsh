; AmyBot Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting AmyBot runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers AmyBot.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if AmyBot.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old AmyBot was installed
  ...
  ; "Can't modify AmyBot's files" retry loop -> "AmyBot 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "AmyBot 无法关闭" dialog.
...
; "AmyBot 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with AmyBot's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove AmyBot application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\amybot (local app data)$\r$\n  • AppData\Roaming\amybot (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering AmyBot processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\amybot"
    RMDir /r "$APPDATA\amybot"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\amybot
    IfFileExists "$LOCALAPPDATA\amybot\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\amybot"
      IfFileExists "$LOCALAPPDATA\amybot\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\amybot"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\amybot
    IfFileExists "$APPDATA\amybot\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\amybot"
      IfFileExists "$APPDATA\amybot\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\amybot"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\amybot\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\amybot"
    IfFileExists "$APPDATA\amybot\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\amybot"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\amybot"
    RMDir /r "$R3\AppData\Roaming\amybot"
